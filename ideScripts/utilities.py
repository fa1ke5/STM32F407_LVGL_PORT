...
def getOpenOcdInterface(openOcdPath):
    '''
    Try to get OpenOCD interface file (TODO: currently hard-coded '') from 'openocd.exe' (openOcdPath) path.
    ...
    if openOcdInterfacePath is None:
        openOcdInterfacePath = getUserPath(" interface")
...
def getOpenOcdConfig(openOcdInterfacePath):
    '''
    Get openOCD configuration files from user, eg. 'interface/, target/stm32f0x.cfg'
